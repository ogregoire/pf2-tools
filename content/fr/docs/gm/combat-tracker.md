...
- **Ajoutez les combattants** avec leur nom, leur initiative, leur CA, leurs PV maximum, leur type (joueur, ennemi, allié ou autre) et un nombre pour ajouter plusieurs créatures identiques d'un coup (elles sont numérotées automatiquement). La liste est triée par initiative au début du combat ; utilisez les flèches ↑/↓ pour réordonner à tout moment (par exemple quand quelqu'un Retarde).
